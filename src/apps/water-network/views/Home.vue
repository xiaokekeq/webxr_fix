...
const appStore = useWaterNetworkAppStore();
const task = computed( () => ui.value.dashboard.task );
const site = computed( () => ui.value.sites.find( ( item ) => item.id === appStore.selectedSiteId ) ?? ui.value.sites[ 0 ] );
...
<template>
	<main class="page home-page">
		...
		<section class="card task-card"><div class="section-heading"><h2>{{ ui.dashboard.taskTitle }}</h2><span class="tag tag-medium">进行中</span></div><h3>{{ task.title }}</h3><p>{{ task.section }}</p><div class="task-meta"><span>{{ task.patrolRange }}</span><span>{{ task.team }} · {{ task.teamCount }} 人</span></div><button class="btn-primary" type="button" @click="navigate('/ar')">{{ ui.application.arEntryLabel }}</button></section>
		...
		<section class="card route-card" @click="navigate('/map')"><div class="section-heading"><h2>{{ ui.dashboard.routeTitle }}</h2><span>查看地图 ›</span></div><p>{{ ui.dashboard.route.distance }} · 预计 {{ ui.dashboard.route.duration }} · {{ ui.dashboard.route.checkpointCount }} 个巡检点</p><div class="route-line"><i></i><i></i><i></i></div></section>
