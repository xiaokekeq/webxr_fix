...
const productBasePath = computed( () => route.meta.product === 'water-network' ? '/water-network' : '/dam' );
const tabs = computed<TabItem[]>( () => [
	{ key: 'home', label: TEXT.home, path: productBasePath.value, icon: '⌂' },
	{ key: 'ar', label: TEXT.ar, path: `${productBasePath.value}/ar`, icon: 'AR', autoStart: true },
	{ key: 'map', label: TEXT.map, path: `${productBasePath.value}/map`, icon: '◫' },
	{ key: 'records', label: TEXT.records, path: `${productBasePath.value}/records`, icon: '☰' },
	{ key: 'profile', label: TEXT.profile, path: `${productBasePath.value}/profile`, icon: '◉' }
] );
const activePath = computed( () => tabs.value.find( ( item ) => route.path === item.path || route.path.startsWith( `${item.path}/` ) )?.path ?? productBasePath.value );
